...
class AssetType{
    ...
    public:
        ...
        bool IS_RAMLOADED(){return RAMLOADED;};// ram loaded et vram loaded sont indépendant
        bool IS_VRAMLOADED(){return VRAMLOADED;};
        //virtual AssetType(DataType Data);
        virtual bool IsMutable(){return false;};//renvoie true si les altération sur les donné on un sens
...
class ParentAsset : public ParentAsset_LEGACY<REAL_ASSET_TYPE>,public ParentAsset_UNTYPED{
    ...
    public:
        ...
        void AskLoadVRAM(){
            VRAM_LoadDemand++;
            VRAM_UnloadDemand--;
            if(AssessVote(VRAM_LoadDemand,VRAM_UnloadDemand,IsVRAMLoaded()) && !IsVRAMLoaded()){//si le vote gagne et que ce n'est pas déja appliqué
                LoadVRAM();
            }
        }
        void AskUnloadVRAM(){
